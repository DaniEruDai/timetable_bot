...
class RecipientGoogleSheets:
    ...
    def __init__(self, tableid: str, encode: str = 'utf-8', save_directory: str = 'result.csv'):
        ...
        self.df = pd.read_csv(self.URL).astype(str)
    ...
    def get_all(self):
        ...
        all_table_rows = self.df.values.tolist()
        all_table_columns = []
        for column_num in range(len(max(all_table_rows))):
            column = [str(i[column_num]) for i in all_table_rows]
            all_table_columns.append(column)
    ...
    def get_all_rows(self):
        return self.df.values.astype(str)

    def get_cell(self, column_num: int, line_num: int) -> str:
        return str(self.get_column(column_num)[line_num])

    def indexes(self, word):
        horizontal_index = [line for line in self.df.values if f'{word}' in line][0].tolist()
        vertical_index = self.df.values.tolist().index(horizontal_index)
        return horizontal_index.index(word), vertical_index
